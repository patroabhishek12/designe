...
Branch B1 data spans multiple bulkheads (failure domains), so the gateway must return a single paginated result stream while preserving high availability, fault tolerance, and ordering guarantees.
